...
%    & \pbox[c]{\textwidth}{\includegraphics[scale=.3]{sims-10-4.pdf}}
  ...
  \end{tabular}
\end{center}
\caption{{\footnotesize Simulated distribution of binomial ordering preferences for a single expression type with $\mu=0.6$ in a standard 2-Alternative Iterated Learning Model (dotted lines) and one with an explicit regularization bias (solid lines)\label{fig:R11}. Note that $\theta'_1=\theta_1$ in the standard model. Regularization depends upon $N$ only in the model with an explicit regularization bias.}}
\end{figure}
