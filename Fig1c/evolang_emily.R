...
plot.spline.contribution <- function(m,x.raw,y.raw,spline.basis,idx,x.axis,xlab=NULL,ylab=NULL,ylim=NULL,lwd=lwd,as.spline=T,x.discrete=T,...) {
        ...
        par(mar=c(-0.1,4.5,4,2)+0.1)
        plot(mle.contribution ~ x.axis,type="l",xlab="",xaxt="n",ylab=ylab,ylim=ylim,lwd=lwd,las=1,...)
        points(x.raw,y.raw,pch=".")
        lines(mle.contribution + ci.size ~ x.axis, lty=2, lwd=lwd)
        lines(mle.contribution - ci.size ~ x.axis, lty=2, lwd=lwd)
        ...
        if(x.discrete) {
                ...
                plot(density(x.raw),ylab="Freq",xlab=xlab,main="",lwd=lwd,las=1,...)      
        }
}
...
plot.spline.contribution(spline.model,log.m,corpusdata$extremity,1:5,x.axis=x,x.discrete=F,ylim=c(0,0.5),ylab="Extremity",xlab="Log overall freq",cex.axis=2,cex.lab=2,lwd=3)
dev.off()
